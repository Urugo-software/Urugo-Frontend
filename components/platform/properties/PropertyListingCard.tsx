...
function PropertyListingCard({
  ...
  return (
    <article className="group max-lg:flex-col  flex gap-4 rounded-sm border border-brand/10 bg-white p-4 transition-all duration-200 hover:border-brand/40 hover:shadow-[0_10px_30px_-18px_rgba(29,78,216,.35)]">
      {/* Property image */}
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            ...
            className="shrink-0 rounded-md p-1 text-faint transition hover:bg-surface hover:text-ink"
          >
            {/* Price */}
            <div>
        ...
        <div className="mt-5 flex flex-col items-start justify-between gap-4">
          <div className=" flex flex-wrap items-center gap-x-4 gap-y-2">
            ...
            {beds > 0 && (
              <span className="flex items-center gap-1.5 text-xs text-body">
                <BedDouble className="h-3.5 w-3.5 text-faint" />
            ...
            {baths > 0 && (
              <span className="flex items-center gap-1.5 text-xs text-body">
                <Bath className="h-3.5 w-3.5 text-faint" />
            ...
            {parking && (
              <span className="flex items-center gap-1.5 text-xs text-body">
                <CarFront className="h-3.5 w-3.5 text-faint" />
                Parking
              </span>
            )}

            <span
              className={`flex items-center gap-1 text-[11px] font-semibold ${isVerified ? "text-brand" : "text-red-600"}`}
            >
              {isVerified ? (
          ...
          <span className=" bg-white/95 px-2 py-1 text-xs border border-brand/40 font-bold text-brand-deep uppercase ">
            {availabilityStatus}
          </span>
        </div>
      </div>
